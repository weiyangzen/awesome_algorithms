"""Best-Fit memory allocation MVP.

This script simulates contiguous dynamic partition allocation in an OS:
- `allocate(pid, size)`: pick the smallest free block that can hold `size`
...
class BestFitAllocator:
    """Contiguous allocator using the best-fit policy."""
    ...
    def allocate(self, pid: str, size: int) -> int | None:
        ...
        if remaining == 0:
            ...
            self.blocks[best_index : best_index + 1] = [allocated, residue]
        return alloc_start
...
def print_state(allocator: BestFitAllocator, title: str) -> None:
    print(f"\n=== {title} ===")
...
def main() -> None:
    allocator = BestFitAllocator(total_memory=1024)
    ...
    plan: list[tuple[str, str, int | None]] = [
        ...
        ("free", "B", None),
        ("free", "D", None),
        ("alloc", "E", 80),   # best-fit picks former D block (size 90)
        ("alloc", "F", 260),  # best-fit picks former B block (size 300)
        ("alloc", "G", 50),   # may fail if no suitable contiguous free block
    ]
