"""Minimal runnable MVP for Euler trail/circuit (MATH-0481).

This script implements Hierholzer's algorithm from scratch for both
undirected and directed graphs, including existence checks and result
verification.
"""
...
def is_connected_nonzero_undirected(n: int, adj: Sequence[Sequence[Tuple[int, int]]], degree: Sequence[int]) -> bool:
    """Check whether all non-zero-degree vertices are in one component."""
...
def is_weakly_connected_nonzero_directed(
    ...
    """Check weak connectivity among vertices with non-zero total degree."""
...
def hierholzer_undirected(n: int, edges: Sequence[Edge], start: int) -> List[int]:
    """Construct Euler trail/circuit in an undirected multigraph."""
    m = len(edges)
...
def hierholzer_directed(n: int, edges: Sequence[Edge], start: int) -> List[int]:
    """Construct Euler trail/circuit in a directed multigraph."""
    m = len(edges)
...
def find_euler_directed(n: int, edges: Sequence[Edge]) -> Tuple[str, List[int]]:
    ...
    if len(balanced) + len(starts) + len(ends) != n:
        return "none", []
...
def verify_undirected_path(edges: Sequence[Edge], path: Sequence[int]) -> bool:
    """Verify path uses every undirected edge exactly once."""
...
def verify_directed_path(edges: Sequence[Edge], path: Sequence[int]) -> bool:
    """Verify path uses every directed edge exactly once with direction."""
    if len(path) != len(edges) + 1:
        return False
...
def run_demo_cases() -> None:
    ...
    print("Euler Trail/Circuit MVP (MATH-0481)")
    print("=" * 72)

    undirected_circuit = np.array(
        [
            [0, 1],
            [1, 2],
            [2, 0],
    ...
    for name, n, edges, expected in und_cases:
    ...
    for name, n, edges, expected in dir_cases:
