...
def validate_inputs(A: np.ndarray, b: np.ndarray, tol: float, max_iter: int) -> None:
    ...
    if np.any(np.isclose(np.diag(A), 0.0)):
        raise ValueError("A has a zero (or near-zero) diagonal entry; Jacobi is not defined.")
    if tol <= 0.0:
...
def jacobi_solve(
    ...
    """Solve Ax=b using Jacobi iteration with infinity-norm step stopping."""
    validate_inputs(A=A, b=b, tol=tol, max_iter=max_iter)
...
def build_strictly_diagonally_dominant_system(n: int, seed: int = 66) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Build a reproducible strictly diagonally dominant linear system Ax=b."""
    if n <= 1:
    ...
    # Enforce strict diagonal dominance to guarantee Jacobi convergence.
...
def main() -> None:
    ...
    A, b, x_true = build_strictly_diagonally_dominant_system(n=n, seed=66)
