"""Minimal runnable MVP for Transformer (MATH-0313).

This demo implements a tiny, source-transparent encoder-decoder Transformer
without calling torch.nn.Transformer as a black box.

Task: sequence reversal
- source: random digit sequence, e.g. [2, 7, 5, 1]
- target: reversed sequence, e.g. [1, 5, 7, 2]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if attn_mask is not None:
            # attn_mask shape should be broadcastable to [B, H, Q, K].
...
def tensorize_pairs(
    ...
    for src_seq, tgt_seq in pairs:
        ...
        if len(src_tokens) > max_len or len(tgt_in_tokens) > max_len or len(tgt_out_tokens) > max_len:
            raise ValueError("max_len is too small for the generated sequence")

        src_tokens = src_tokens + [PAD_ID] * (max_len - len(src_tokens))
        tgt_in_tokens = tgt_in_tokens + [PAD_ID] * (max_len - len(tgt_in_tokens))
        tgt_out_tokens = tgt_out_tokens + [PAD_ID] * (max_len - len(tgt_out_tokens))
...
def greedy_decode(
    model: TransformerSeq2Seq,
    src_seq: Sequence[int],
    max_len: int,
) -> List[int]:
    """Autoregressive greedy decoding until EOS or max length."""
    model.eval()
...
def evaluate_exact_match(
    ...
    """Compute exact match ratio on test pairs."""
    correct = 0
    for src_seq, tgt_seq in pairs:
...
def main() -> None:
    print("Transformer MVP (MATH-0313)")
    print("=" * 72)
    ...
    max_content_len = 7
    max_len = max_content_len + 1  # +1 for EOS/BOS alignment.

    train_pairs = build_reverse_dataset(num_samples=512, min_len=3, max_len=max_content_len, rng=rng)
    ...
    for idx, (src_seq, tgt_seq) in enumerate(test_pairs[:5], start=1):
        pred_seq = greedy_decode(model=model, src_seq=src_seq, max_len=max_len)
        print(f"  case {idx}: src={src_seq} pred={pred_seq} tgt={list(tgt_seq)}")

    # Deterministic quality gates on this toy task.
    assert losses[0] > losses[-1], "loss did not decrease"
    assert exact_match >= 0.80, "exact match is unexpectedly low"
