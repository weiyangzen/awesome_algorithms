"""SJF scheduling MVP (single-core, non-preemptive, no I/O blocking)."""
...
def sjf_non_preemptive(
    ...
    """Run non-preemptive SJF and return (timeline, per-process metrics, averages)."""
    ...
    ready_heap: List[Tuple[int, int, str, int]] = []
    ...
    def enqueue_arrived() -> None:
        ...
        while next_idx < n and processes[next_idx].arrival <= time:
            ...
            # heap key: shortest burst first, then earlier arrival, then lexical pid.
            heapq.heappush(ready_heap, (p.burst, p.arrival, p.pid, next_idx))
            next_idx += 1
    ...
    while completed < n:
        ...
        _, _, _, idx = heapq.heappop(ready_heap)
...
def main() -> None:
    ...
    timeline, metrics, avg = sjf_non_preemptive(process_specs)

    print("SJF Scheduling Demo (Non-preemptive)")
    print()
    print("Timeline:")
