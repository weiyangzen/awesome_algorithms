"""1D MacCormack MVP for linear advection with periodic boundary."""
...
def initial_condition_smooth(x: np.ndarray) -> np.ndarray:
    ...
    gaussian = np.exp(-((x - 0.35) ** 2) / 0.004)
    sine = 0.20 * np.sin(2.0 * np.pi * x)
    return gaussian + sine


def initial_condition_square(x: np.ndarray) -> np.ndarray:
    """Discontinuous profile for oscillation diagnostic."""
    return np.where((x >= 0.2) & (x <= 0.5), 1.0, 0.0)


def maccormack_step(u: np.ndarray, cfl: float) -> np.ndarray:
    """Advance one step by MacCormack predictor-corrector scheme.

    For positive velocity (cfl >= 0):
      predictor uses forward difference, corrector uses backward difference.
    For negative velocity (cfl < 0):
      predictor uses backward difference, corrector uses forward difference.
    """
    if u.ndim != 1:
    ...
    if cfl >= 0.0:
        ...
        u_pred = u - cfl * (np.roll(u, -1) - u)
        # Corrector: backward difference on predicted field
        u_next = 0.5 * (u + u_pred - cfl * (u_pred - np.roll(u_pred, 1)))
    else:
        # Predictor: backward difference
        u_pred = u - cfl * (u - np.roll(u, 1))
        # Corrector: forward difference on predicted field
        u_next = 0.5 * (u + u_pred - cfl * (np.roll(u_pred, -1) - u_pred))

    return ensure_finite_array("u_next", u_next)


def solve_maccormack(
    nx: int,
    a: float,
    t_end: float,
    cfl_target: float,
...
def run_resolution_case(nx: int, a: float, t_end: float, cfl_target: float) -> ResolutionResult:
    ...
    x, u0, u_num, cfl, n_steps, _dt = solve_maccormack(
        nx=nx,
        a=a,
        t_end=t_end,
...
def main() -> None:
    ...
    print("=== MacCormack MVP: 1D Linear Advection ===")
    ...
    if p1 < 1.50 or p2 < 1.50:
        raise AssertionError("Observed order is too low for expected second-order behavior")

    # Extra diagnostic on discontinuous data to show dispersive behavior.
    nx_diag = 400
    _x, u0_sq, u_sq, cfl_sq, n_steps_sq, _dt = solve_maccormack(
        nx=nx_diag,
        a=a,
        t_end=t_end,
        cfl_target=cfl_target,
    ...
    mass_error_sq = float(np.sum(u_sq - u0_sq) * (1.0 / nx_diag))

    print("\nDiscontinuous initial condition diagnostic")
    print(f"nx={nx_diag}, n_steps={n_steps_sq}, actual_cfl={cfl_sq:.6f}")
    print(
        f"TV0={tv0:.6f}, TVT={tvt:.6f}, min(u)={u_sq.min():.6f}, max(u)={u_sq.max():.6f}"
    )
    print(f"mass_error={mass_error_sq:.2e}")

    if not np.all(np.isfinite(u_sq)):
        raise AssertionError("Non-finite values detected in discontinuous diagnostic")

    print("\nAll MacCormack MVP checks passed.")
