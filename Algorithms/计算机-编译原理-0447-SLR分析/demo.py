"""Canonical SLR(1) parser MVP for CS-0287.
...
def build_expression_grammar() -> tuple[Grammar, str]:
    """Build a classic expression grammar that is SLR(1)."""
    grammar: Grammar = {
        "S": [("E",)],
        "E": [("E", "+", "T"), ("T",)],
        "T": [("T", "*", "F"), ("F",)],
        ...
        for lhs, productions in grammar.items():
            for rhs in productions:
                for idx, symbol in enumerate(rhs):
                    ...
                    follow_sets[symbol] |= beta_first - {EPS}
                    if len(follow_sets[symbol]) != old_size:
                        changed = True

                    if not beta or EPS in beta_first:
                        old_size = len(follow_sets[symbol])
...
def closure_lr0(items: frozenset[LR0Item], grammar: Grammar) -> frozenset[LR0Item]:
    nonterminals = set(grammar.keys())
    ...
    while changed:
        ...
        for item in closure:
            ...
            for prod_rhs in grammar[symbol]:
                candidate = LR0Item(lhs=symbol, rhs=prod_rhs, dot=0)
...
def goto_lr0(items: frozenset[LR0Item], symbol: str, grammar: Grammar) -> frozenset[LR0Item]:
    moved: set[LR0Item] = set()

    for item in items:
        if item.dot < len(item.rhs) and item.rhs[item.dot] == symbol:
            moved.add(LR0Item(lhs=item.lhs, rhs=item.rhs, dot=item.dot + 1))

    if not moved:
        return frozenset()
    return closure_lr0(frozenset(moved), grammar)


def canonical_lr0_collection(
    grammar: Grammar,
    aug_start: str,
    terminals: set[str],
    nonterminals: set[str],
) -> tuple[list[frozenset[LR0Item]], dict[tuple[int, str], int]]:
    start_item = LR0Item(lhs=aug_start, rhs=grammar[aug_start][0], dot=0)
    start_state = closure_lr0(frozenset({start_item}), grammar)
    ...
    while cursor < len(states):
        state = states[cursor]
        for symbol in symbols:
            nxt = goto_lr0(state, symbol, grammar)
            if not nxt:
                continue
    ...
    for sid, state in enumerate(states):
        for item in state:
            ...
            if item.lhs == aug_start:
                ...
                for terminal in sorted(follow_sets[item.lhs]):
                    set_action(sid, terminal, ("reduce", prod_idx))
...
def print_first_follow(
    ...
    for nt in ordered_nonterminals:
        first_text = "{" + ", ".join(sorted(first_sets[nt])) + "}"
        follow_text = "{" + ", ".join(sorted(follow_sets[nt])) + "}"
        print(f"  FIRST({nt}) = {first_text:<20} FOLLOW({nt}) = {follow_text}")


def print_states(states: list[frozenset[LR0Item]], max_states: int = 10) -> None:
    print(f"\nCanonical LR(0) items (showing up to {max_states} states):")
...
def run_demo() -> None:
    ...
    follow_sets = compute_follow_sets(augmented_grammar, aug_start, first_sets)

    lr0_states, lr0_transitions = canonical_lr0_collection(
    ...
    action_table, goto_table, conflicts = build_slr_tables(
        states=lr0_states,
        transitions=lr0_transitions,
    ...
    print_first_follow(first_sets, follow_sets, ordered_nonterminals)
    print_states(lr0_states)
    ...
    if conflicts:
        ...
        for conflict in conflicts:
            print(f"  - {conflict}")
        raise RuntimeError("SLR table has conflicts")
    ...
    print("\nSummary:")
    print(f"  Canonical LR(0) states for SLR: {state_count}")
