"""Minimal runnable MVP for Gauss-Seidel iteration."""
...
class GaussSeidelResult:
    """Container for Gauss-Seidel outputs and diagnostics."""
...
def validate_inputs(
    ...
    if np.any(np.isclose(np.diag(A), 0.0)):
        raise ValueError(
            "A has a zero (or near-zero) diagonal entry; Gauss-Seidel is undefined."
        )
...
def gauss_seidel_solve(
    ...
    """Solve Ax=b by Gauss-Seidel iteration using infinity-norm step criterion."""
    ...
    for k in range(1, max_iter + 1):
        ...
        for i in range(n):
            sigma_lower = float(np.dot(A[i, :i], x[:i]))
            sigma_upper = float(np.dot(A[i, i + 1 :], x_old[i + 1 :]))
            x[i] = (b[i] - sigma_lower - sigma_upper) / A[i, i]

        step_error_inf = float(np.linalg.norm(x - x_old, ord=np.inf))
        iterations = k
    ...
    # Iteration matrix T_GS = -(D+L)^{-1} U.
    D = np.diag(np.diag(A))
    L = np.tril(A, k=-1)
    U = np.triu(A, k=1)
    T_GS = -np.linalg.solve(D + L, U)
    spectral_radius_T = float(np.max(np.abs(np.linalg.eigvals(T_GS))))
    ...
    return GaussSeidelResult(
        ...
        spectral_radius_T=spectral_radius_T,
    )
...
def build_strictly_diagonally_dominant_system(
    ...
    """Build a reproducible strictly diagonally dominant linear system."""
    if n <= 1:
...
def run_checks(result: GaussSeidelResult, x_true: np.ndarray, x_ref: np.ndarray) -> None:
    ...
    if not result.converged:
        raise AssertionError("Gauss-Seidel did not converge within max_iter.")
    if result.spectral_radius_T >= 1.0:
        raise AssertionError(
            f"Spectral radius condition violated: rho(T_GS)={result.spectral_radius_T:.6f}"
        )
    if result.residual_l2 > 1e-10:
        raise AssertionError(f"Residual too large: {result.residual_l2:.3e}")
    if err_ref_inf > 1e-10:
        raise AssertionError(f"Deviation from numpy solve too large: {err_ref_inf:.3e}")
    if err_true_inf > 1e-10:
...
def main() -> None:
    ...
    A, b, x_true = build_strictly_diagonally_dominant_system(n=n, seed=491)
    result = gauss_seidel_solve(A=A, b=b, x0=None, tol=tol, max_iter=max_iter)
    ...
    print("Gauss-Seidel iteration MVP report")
    print(f"matrix_size                    : {n}")
    print(f"max_iter                       : {max_iter}")
    print(f"tol                            : {tol:.1e}")
    print(f"iterations_used                : {result.iterations}")
    print(f"converged                      : {result.converged}")
    print(f"spectral_radius_T              : {result.spectral_radius_T:.6f}")
