"""Canonical LR(0) parser MVP for CS-0286.
...
def build_lr0_demo_grammar() -> tuple[Grammar, str]:
    """Build a classic LR(0) grammar.
...
def build_lr0_tables(
    ...
    all_action_terminals = sorted(terminals | {END})
    ...
    for sid, state in enumerate(states):
        for item in state:
            ...
            if item.lhs == aug_start:
                ...
                for terminal in all_action_terminals:
...
def print_states(states: list[frozenset[LR0Item]], max_states: int = 12) -> None:
...
def print_action_goto_tables(
    ...
    print("\nLR(0) ACTION table:")
    action_symbols = terminals + [END]
    header = ["state"] + action_symbols
    widths = [max(5, len(h)) for h in header]

    for sid in range(num_states):
        widths[0] = max(widths[0], len(str(sid)))
        for idx, t in enumerate(action_symbols, start=1):
...
def run_demo() -> None:
    grammar, start_symbol = build_lr0_demo_grammar()
    ...
    states, transitions = canonical_lr0_collection(
    ...
    action_table, goto_table, conflicts = build_lr0_tables(
        states=states,
    ...
    if conflicts:
        ...
        for conflict in conflicts:
            print(f"  - {conflict}")
        raise RuntimeError("Grammar is not LR(0): conflicts detected")

    cases: list[tuple[str, list[str], bool]] = [
        ("valid_1", ["d", "d"], True),
        ("valid_2", ["c", "d", "d"], True),
        ("valid_3", ["c", "c", "d", "d"], True),
        ("valid_4", ["d", "c", "d"], True),
        ("invalid_1", ["d"], False),
        ("invalid_2", ["c", "d", "c"], False),
        ("invalid_3", ["c", "c", "d"], False),
    ]
    ...
    print("\nSummary:")
    print(f"  Canonical LR(0) states: {state_count}")
