...
CoreItem = tuple[str, Production, int]
...
def build_expression_grammar() -> tuple[Grammar, str]:
    """Build a classic expression grammar that is LALR(1)."""
    grammar: Grammar = {
        "S": [("E",)],
        "E": [("E", "+", "T"), ("T",)],
        "T": [("T", "*", "F"), ("F",)],
...
def core_of_state(state: frozenset[LR1Item]) -> frozenset[CoreItem]:
    return frozenset((item.lhs, item.rhs, item.dot) for item in state)
...
def merge_lalr_states(
    ...
    core_to_gid: dict[frozenset[CoreItem], int] = {}
    groups: list[list[int]] = []

    for sid, state in enumerate(lr1_states):
        core = core_of_state(state)
        gid = core_to_gid.get(core)
        if gid is None:
            gid = len(groups)
            core_to_gid[core] = gid
            groups.append([])
    ...
    for gid, members in enumerate(groups):
        ...
        merged_lookaheads: dict[CoreItem, set[str]] = {}
        for sid in members:
            for item in lr1_states[sid]:
                core_item: CoreItem = (item.lhs, item.rhs, item.dot)
                merged_lookaheads.setdefault(core_item, set()).add(item.lookahead)

        merged_items: set[LR1Item] = set()
        for (lhs, rhs, dot), las in merged_lookaheads.items():
            for lookahead in sorted(las):
                merged_items.add(LR1Item(lhs=lhs, rhs=rhs, dot=dot, lookahead=lookahead))
    ...
    for (sid, symbol), tid in lr1_transitions.items():
        g_from = old_to_new[sid]
        g_to = old_to_new[tid]

        key = (g_from, symbol)
        existing = merged_transitions.get(key)
        if existing is not None and existing != g_to:
            inconsistencies.append(
                f"transition inconsistency at merged state {g_from} on '{symbol}': {existing} vs {g_to}"
            )
            continue
        merged_transitions[key] = g_to

    return merged_states, merged_transitions, old_to_new, inconsistencies
...
def print_states(states: list[frozenset[LR1Item]], title: str, max_states: int = 8) -> None:
...
def run_demo() -> None:
    ...
    lalr_states, lalr_transitions, old_to_new, merge_warnings = merge_lalr_states(
        lr1_states, lr1_transitions
    )

    action_table, goto_table, conflicts = build_lalr_tables(
        states=lalr_states,
    ...
    print_states(lalr_states, "Merged LALR(1) items")
    ...
    if merge_warnings:
        ...
        for warning in merge_warnings:
            print(f"  - {warning}")
        raise RuntimeError("LALR merge generated inconsistent transitions")

    if conflicts:
        print("\nConflicts:")
        for conflict in conflicts:
            print(f"  - {conflict}")
        raise RuntimeError("LALR(1) table has conflicts")

    print("\nState merge mapping (canonical LR(1) state -> LALR state):")
    for sid in range(len(lr1_states)):
        print(f"  {sid} -> {old_to_new[sid]}")
    ...
    canonical_state_count = len(lr1_states)
    lalr_state_count = len(lalr_states)
    compression_ratio = float(
        np.array([lalr_state_count / canonical_state_count], dtype=np.float64)[0]
    )

    action_slots = lalr_state_count * (len(terminals) + 1)
    ...
    print("\nSummary:")
    print(f"  Canonical LR(1) states: {canonical_state_count}")
    print(f"  LALR(1) states: {lalr_state_count} (compression ratio={compression_ratio:.2%})")
