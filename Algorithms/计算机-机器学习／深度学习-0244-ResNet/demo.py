"""ResNet minimal runnable MVP on sklearn digits.

The residual blocks are implemented explicitly with PyTorch modules.
...
class BasicBlock(nn.Module):
    """Minimal residual block: Conv-BN-ReLU -> Conv-BN + shortcut -> ReLU."""
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1) -> None:
        ...
        if stride == 1 and in_channels == out_channels:
            ...
            # Projection shortcut to match both spatial size and channel size.
...
class TinyResNet(nn.Module):
    """Small ResNet-style classifier for 8x8 grayscale digit images."""
    ...
    def __init__(self, num_classes: int = 10) -> None:
        ...
        self.layer1 = self._make_layer(out_channels=16, blocks=2, stride=1)
        self.layer2 = self._make_layer(out_channels=32, blocks=2, stride=2)
...
def main() -> None:
    ...
    print(f"model: TinyResNet (params={sum(p.numel() for p in model.parameters())})")
    ...
    if final_test_acc < baseline_acc + 0.40:
        raise RuntimeError(
            "model did not beat majority baseline by >= 0.40: "
