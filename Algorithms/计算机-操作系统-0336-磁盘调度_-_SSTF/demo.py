"""SSTF disk scheduling MVP with deterministic demo and lightweight statistics."""
...
def sstf_schedule(
    ...
    """Shortest Seek Time First with deterministic tie-break (smaller cylinder first)."""
    ...
    while pending:
        # Pick nearest request; if tied, pick smaller cylinder for reproducibility.
        best_idx = 0
        best_req = pending[0]
        ...
        rows.append(
            {
                ...
                "improvement": fcfs_res.total_seek - sstf_res.total_seek,
            }
        )
...
def main() -> None:
    ...
    # Deterministic checks for built-in sample.
    ...
    assert sorted(sstf_res.service_order) == sorted(requests)
    assert len(sstf_res.service_order) == len(requests)

    sstf_moves_df = moves_to_dataframe(sstf_res.moves)
    fcfs_moves_df = moves_to_dataframe(fcfs_res.moves)

    print("=== SSTF Disk Scheduling Demo ===")
    print(f"requests = {requests}")
    print(f"start_head = {start_head}, max_cylinder = {max_cylinder}")
    print()

    print("SSTF service order:")
    print(sstf_res.service_order)
    print()

    print("SSTF movement trace:")
        ...
        [
            {
                ...
                "avg_seek_per_request": sstf_res.average_seek_per_request,
            },
            {
                "algorithm": "FCFS",
    ...
    better_ratio = float((random_df["sstf_seek"] < random_df["fcfs_seek"]).mean())

    print("Random workload (200 trials) average total seek:")
    print(f"SSTF={mean_sstf:.3f}, FCFS={mean_fcfs:.3f}")
    print(f"SSTF better ratio={better_ratio:.3f}")

    assert len(random_df) == 200
    assert 0.0 <= better_ratio <= 1.0
    assert mean_sstf >= 0 and mean_fcfs >= 0
